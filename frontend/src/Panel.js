...
import { Trash2, Power, PowerOff, RotateCcw, CheckCircle,RefreshCcw } from "lucide-react";
  ...
  return (
    ...
    <div className="p-6 min-h-screen bg-gray-950 text-gray-100">
      <div className="flex justify-between items-start mb-6 flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white-700 mb-1">Panel de Infraestructura IT</h1>
          {usuario && (
...
{servidores.length > 0 ? (
  <>
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {servidores
        ...
        .map((srv) => (
          <div key={srv.id} className="bg-gray-900 p-4 rounded-lg shadow border border-gray-900">
  ...
  <strong>IP:</strong> <span className="text-gray-100">{srv.ip}</span>
</p>

<p className="flex items-center gap-2">
  <MdOutlinePower className={srv.estado === "online" ? "text-green-400" : "text-red-400"} />
  <strong>Estado:</strong>{" "}
  <span className={srv.estado === "online" ? "text-green-500" : "text-red-500"}>
    {srv.estado}
  </span>
</p>
...
<p className="flex items-center gap-2">
  ...
  <strong>CPU:</strong> <span className="text-gray-100">{srv.cpu}</span>
</p>

<p className="flex items-center gap-2">
  <FaMemory className="text-purple-400" />
  <strong>RAM:</strong> <span className="text-gray-100">{srv.ram}</span>
</p>
  ...
  <label className="relative inline-flex items-center cursor-pointer">
    ...
    `}></div>
    <div className={`absolute left-0.5 top-0.5 w-5 h-5 bg-white rounded-full shadow-md transition-all duration-300
...
<button
  ...
  disabled={srv.estado !== "online" || loadingBotones[srv.id]}
...
>
  <svg
    ...
    strokeWidth="1.5"
    stroke="currentColor"
    className={`w-5 h-5 ${loadingBotones[srv.id] ? "animate-spin" : ""}`}
  >
    <path
      strokeLinecap="round"
...
<div className="mt-12">
  <div className="flex justify-between items-center mb-2">
    <h2 className="text-xl font-bold text-gray-100">Historial de eventos</h2>
    <button
      onClick={() => {
        setEventos([]);
        localStorage.removeItem("eventos");
